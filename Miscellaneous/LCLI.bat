...
:installLC
echo Minecraft/LunarClient not found, press a key to automatically download and run it or close this window.
pause >nul
set LunarVer=v2.7.4
if exist "%temp%\Lunar_%LunarVer%.exe" goto:runLC
if exist "%temp%\Lunar.exe" del /F /Q "%temp%\Lunar.exe"
powershell Invoke-WebRequest "https://launcherupdates.lunarclientcdn.com/Lunar%20Client%20%LunarVer%.exe" -OutFile "%temp%\Lunar_%LunarVer%.exe"
:runLC
%temp%\Lunar_%LunarVer%.exe
...
echo '%version%' is incorrect, either pick '7' for 1.7.10, '8' for 1.8.9, '12' for 1.12..
pause >nul
goto :choose
